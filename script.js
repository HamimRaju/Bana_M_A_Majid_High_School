...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const openButtons = document.querySelectorAll(".open-register-overlay");
  const regBackdrop = document.getElementById("regBackdrop");

  const regRegisterCard = document.getElementById("regRegisterCard");
  const regLoginCard = document.getElementById("regLoginCard");

  const regRegisterForm = document.getElementById("regRegisterForm");
  const regLoginForm = document.getElementById("regLoginForm");

  const regCloseBtn = document.getElementById("regCloseBtn");
  const regLoginCloseBtn = document.getElementById("regLoginCloseBtn");
  const regBackBtn = document.getElementById("regBackBtn");

  const regOpenLoginCard = document.getElementById("regOpenLoginCard");
  const regBackToRegister = document.getElementById("regBackToRegister");
  const regNewAccount = document.getElementById("regNewAccount");

  const regPasswordInput = document.getElementById("regPasswordInput");
  const regEyeBtn = document.getElementById("regEyeBtn");

  const regLoginBtn = document.getElementById("regLoginBtn");
  const regGoogleBtn = document.getElementById("regGoogleBtn");
  const regAppleBtn = document.getElementById("regAppleBtn");
  const regFacebookBtn = document.getElementById("regFacebookBtn");
  ...
  function openRegOverlay() {
    ...
    if (regRegisterCard && regLoginCard) {
      regRegisterCard.style.display = "block";
      regLoginCard.style.display = "none";
  ...
  function showRegLoginCard() {
    if (!regRegisterCard || !regLoginCard) return;
    regRegisterCard.style.display = "none";
    regLoginCard.style.display = "block";
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  function showRegRegisterCard() {
    if (!regRegisterCard || !regLoginCard) return;
    regLoginCard.style.display = "none";
    regRegisterCard.style.display = "block";
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  // open overlay from all registration buttons
  ...
  // close overlay
  if (regBackdrop) regBackdrop.addEventListener("click", closeRegOverlay);
  if (regCloseBtn) regCloseBtn.addEventListener("click", closeRegOverlay);
  if (regLoginCloseBtn) regLoginCloseBtn.addEventListener("click", closeRegOverlay);
  if (regBackBtn) {
  ...
  // switch cards
  if (regOpenLoginCard) {
    regOpenLoginCard.addEventListener("click", function (e) {
      e.preventDefault();
      showRegLoginCard();
    });
  }

  if (regBackToRegister) {
    regBackToRegister.addEventListener("click", function (e) {
      e.preventDefault();
      showRegRegisterCard();
    });
  }
  ...
  if (regNewAccount) {
    regNewAccount.addEventListener("click", function (e) {
      e.preventDefault();
      showRegRegisterCard();
    });
  }
  ...
  // forms demo handlers
  if (regRegisterForm) {
    regRegisterForm.addEventListener("submit", function (e) {
      e.preventDefault();
      alert("Register form submitted (demo). এখানে তোমার backend/database কোড বসাও।");
    });
  }

  if (regLoginForm && regLoginBtn) {
    regLoginBtn.addEventListener("click", function () {
      alert("Login button clicked (demo). এখানে তোমার backend call দাও।");
    });
  }

  // password eye toggle
  if (regEyeBtn && regPasswordInput) {
    regEyeBtn.addEventListener("click", function () {
      ...
      const svg = regEyeBtn.querySelector("svg");
      if (!svg) return;
      const slash = svg.querySelector("#regEyeSlash");
      if (slash) slash.style.display = isPassword ? "block" : "none";
    });
  }

  // OAuth popup
  function regOpenOAuth(url) {
    const w = 500, h = 600;
  ...
  if (regGoogleBtn) regGoogleBtn.addEventListener("click", () => regOpenOAuth("https://accounts.google.com/"));
  if (regAppleBtn) regAppleBtn.addEventListener("click", () => regOpenOAuth("https://appleid.apple.com/"));
  if (regFacebookBtn) regFacebookBtn.addEventListener("click", () => regOpenOAuth("https://www.facebook.com/login.php"));
});
